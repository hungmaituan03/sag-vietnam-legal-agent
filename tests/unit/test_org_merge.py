...
def test_org_docs_to_chunks_smoke():
    docs = [
        OrgDocument(org_id="VCC", doc_type="dkkd", text="ĐKKD stub text."),
        OrgDocument(org_id="VCC", doc_type="dieu_le", text="Điều lệ stub."),
    ]
    chunks = org_docs_to_chunks(docs)

    assert len(chunks) == 2
    assert chunks[0].document_id == "org:VCC:dkkd"
    assert chunks[0].chunk_id == "org:VCC:dkkd::body"
    assert chunks[0].text == "ĐKKD stub text."
    assert chunks[0].entity_refs == ["VCC"]
    assert chunks[0].article is None

    assert chunks[1].document_id == "org:VCC:dieu_le"
    assert chunks[1].chunk_id == "org:VCC:dieu_le::body"
...
def test_merge_appends_org_after_statute():
    ...
    org_docs = [
        OrgDocument("VCC", "dkkd", "org A"),
        OrgDocument("VCC", "dieu_le", "org B"),
    ]
    ...
    assert merged[2].document_id == "org:VCC:dkkd"
    assert merged[3].document_id == "org:VCC:dieu_le"
...
def test_merge_max_org_truncates_org_only():
    ...
    assert len(merged) == 2
    assert merged[0].chunk_id == statutes[0].chunk_id
    assert merged[1].document_id == "org:VCC:a"
